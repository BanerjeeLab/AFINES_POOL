...
!------------------------------ Interpolation -----------------------------
	...
	z(i,j)=z(i,j)+exp(-dr/lambda)
        ...
        if(dr.lt.(sqrt(dx*dx+dy*dy))) then		! calculate average cos(theta), sin(theta)
	...
	theta(i,j)=theta(i,j)/z(i,j)					! interpolated theta
	ndir(i,j) = atan2( avgS2/coun(i,j), avgS1/coun(i,j) )   	! get average theta from circular mean
